...
namespace Bis.Controllers
{
    ...
    public class AttendancesController : Controller
    {
        ...
        public JsonResult CategoryEmployees( string date, int category = 0, int shift=0)
        {
            List<object> lstAttendance = new List<object>();
            var catEmployees = db.Employees.Where(x => x.categoryId == category && x.status == "Active");
            //DateTime tm = DateTime.Parse(date);
            ...
            foreach (var employee in catEmployees)
            {
                ...
                if (todayEmployee == null)
                {
                    todayEmployee = new Attendance
                    {
                        ...
                        companyId = Convert.ToInt32(employee.companyId),
                        shiftId = Convert.ToInt32(employee.shiftId),
                        categoryId= Convert.ToInt32(employee.categoryId),
        ...
        public ActionResult SaveAttendance(List<Attendance> attendance, string Date)
        {
            ...
            foreach (var att in attendance)
            {
                ...
                if (todayEmployee != null)
                {
                    ...
                    todayEmployee.PFStatus = att.PFStatus;
                    todayEmployee.entryTime = DateTime.Now;
